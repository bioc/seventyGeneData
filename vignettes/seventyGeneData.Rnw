...
% \VignetteIndexEntry{Working with the seventyGeneData package}
% \VignetteDepends{Biobase, readxl, limma}
% \VignetteKeywords{ExperimentData, Cancer, RNAExpressionData}
% \VignettePackage{seventyGeneData}
...
%%% Sweave options
\SweaveOpts{keep.source=TRUE, print=FALSE}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Title
\title{The seventyGenesData package: annotated gene expression data from the
  van't Veer and Van de Vijver breast cancer cohorts}
...
<<A.start, eval=TRUE, echo=FALSE, cache=FALSE>>=
...
annotate, and then assemble into separate \Rclass{ExpressionSet} instances
the gene expression data of the van't Veer and Van de Vijver cohorts
\cite{van't2002,Vijver2002}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Original data sources}

The original data sets used to develop and validate the 70-genes
signature \cite{van't2002,Vijver2002} were  dowloaded from the website
of the Bioinformatics and Statistics group of the Netherlands Cancer Institute (NKI)
(\url{http://bioinformatics.nki.nl/data.php}), and as supplementary material
associated with the original manuscripts, using the following hyperlinks:

\begin{itemize}
  \item van't Veer cohort (\url{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/}):
    \begin{itemize}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ArrayData_greater_than_5yr.zip}{Training set expression data: good prognosis}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ArrayData_less_than_5yr.zip}{Training set expression data: bad prognosis}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ArrayData_19samples.zip}{Test set expression data}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ArrayData_BRCA1.zip}{Expression data for BRCA1 mutated cases}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ArrayNomenclature_contig_accession.xls}{Microarray feature annotation}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ArrayNomenclature_methods.doc}{Annotation explanation}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/ProbeSeq.xls}{Microarray feature sequences}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/README-Nature_I.doc}{Explanation of files content}
      \item \href{http://bioinformatics.nki.nl/data/van-t-Veer_Nature_2002/codeboek_Rosetta.doc}{Explanation of acronyms used in the files}
      \item \href{http://www.nature.com/nature/journal/v415/n6871/extref/415530a-s8.xls}{Clinical information (from Nature)}
      \item \href{http://www.nature.com/nature/journal/v415/n6871/extref/415530a-s9.xls}{70-gene signature data (from Nature)}
      \end{itemize}
  \item Van de Vijveer cohort:
    \begin{itemize}
    \item \href{http://bioinformatics.nki.nl/data/nejm_table1.zip}{Clinical data}
    \item \href{http://bioinformatics.nki.nl/data/nejm_table3.zip}{The expression data for 70-Gene signature across the 295 samples}
    \item \href{http://bioinformatics.nki.nl/data/nejm_table3.zip}{70-Gene Template for Good Prognosis Signature}
    \item \href{http://bioinformatics.nki.nl/data/ZipFiles295Samples.zip}{Genome-Wide Gene Expression Data for 295 Samples}
  \end{itemize}
\end{itemize}

All the files, among those mentioned above, needed to compile this vignette
and create the \Rpackage{seventyGeneData} \R package are included
inside the \texttt{inst/extdata} directory.

The microarray feature annotation can be obtained from the \Rpackage{breastCancerNKI}
{\R-\Bioc} package, in which the two cohorts are merged in a unique \Rclass{ExpressionSet}
instance. On the contrary in the present {\R-\Bioc} package the two data sets are maintained
separate, the training and test sets of sampes used in the original studies are identified,
along whith the microarray features constituting the 70-gene signature.
...
lapply(urlList, function(x) {
	download.file(x, destfile=paste("./inst/extdata/vantVeer/", gsub(".+/", "", x), sep=""),
...
This code chunk was not run to compile this vignette, which was preapred
using the local copies of the data stored in the \texttt{inst/extdata} directory.
...
lapply(urlList, function(x) {
	download.file(x, destfile=paste("./inst/extdata/vanDeVijver/", gsub(".+/", "", x), sep=""),
		      quiet = FALSE, mode = "w", cacheOK = TRUE)
})
@


Below is the code to download and install the \Rpackage{breastCancerNKI}
annotation metadata package from \Bioc, along with other required libraries
if the are not available (e.g \Rpackage{readxl}).
As shown below the \Rpackage{breastCancerNKI} contains a unique
instance of class \Rclass{ExpressionSet} for both cohorts,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\Rclass{ExpressionSet} preparation}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Feature data preparation}
...
signature identified in the van't Veer study \cite{van't2002},
as available from the supplementary data section of the original publication.
To this end the Excel spreadsheet named \texttt{415530a-s9.xls} corresponds to
...
(``Supplementary Methods'' section, file \texttt{415530a-s7.doc})
...
contained in the \Rpackage{breastCancerNKI}:

<<assembleAnnotation, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleAnnotation2, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Load the library
require(readxl)
...
<<assembleAnnotation3, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Assemble the \Robject{vantVeer } \Rclass{ExpressionSet}}
...
\begin{itemize}
  \item \texttt{ArrayData\_19samples.zip}, containing data for 19 samples (test set);
  \item \texttt{ArrayData\_BRCA1.zip}, containing data for 20 samples (BRCA1 and 2 mutation).
  \item \texttt{ArrayData\_greater\_than\_5yr.zip}, containing data for 51 samples;
  \item \texttt{ArrayData\_less\_than\_5yr.zip}, containing data for 46 samples;
\end{itemize}
...
\begin{itemize}
  \item Log ratio;
  \item P-value for log ratio significance;
  \item Log intensity;
\end{itemize}
...
with the \Rfunction{read_xls} function contained in the \Rpackage{readxl}.
...
and we have provided them with the \Rpackage{seventyGeneData} package
inside the \texttt{inst/extdata} directory.
Finally, the Sample IDs used in such gene expression tables correspond
to those used in the clinical data table (archived in the \texttt{415530a-s8.xls}).

The \R code used to assemble the \Rclass{ExpressionSet} instance for the van't Veer
cohort contained in the \Rpackage{seventyGenesData} {\R-\Bioc} package
is shown below.
...
<<assembleVantVeer, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Load the library
require(Biobase)
require(readxl)
...
filesVtV <- dir(filesVtVloc, full.names=TRUE, pattern="^ArrayData.*txt$")
filesVtV
@

The code chunk below shows how to read and the phenotypic information
associated with the samples analyzed in the can't Veer cohort:

<<assembleVantVeer2, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer3, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer4, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer5, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer6, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<extractColumns, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Define the function
...
<<assembleVantVeer7, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer8, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
...
<<assembleVantVeer9, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer10, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
...
<<assembleVantVeer11, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembleVantVeer12, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
all(phenoVtV$SampleName == colnames(intensity))
@

The code chunk below shows how to create an instance of
class \Rclass{ExpressionSet} for the van't Veer cohort.

<<assembleVantVeer13, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Assemble the \Robject{vanDeVijver } \Rclass{ExpressionSet}}

For the Van de Vijver cohort the gene expression data for 295 samples
is contained in the \texttt{ZipFiles295Samples.zip} compressed archive.
Such data is organized in six separate text files:
\begin{itemize}
  \item \item{Table\_NKI\_295\_1.txt}, containing data for 50 samples;
  \item \texttt{Table\_NKI\_295\_2.txt}, containing data for 50 samples.
  \item \texttt{Table\_NKI\_295\_3.txt}, containing data for 50 samples;
  \item \texttt{Table\_NKI\_295\_4.txt}, containing data for 50 samples;
  \item \texttt{Table\_NKI\_295\_5.txt}, containing data for 50 samples;
  \item \texttt{Table\_NKI\_295\_6.txt}, containing data for 45 samples;
\end{itemize}
...
\begin{itemize}
  \item Log ratio;
  \item Log ratio error;
  \item P-value for log ratio significance;
  \item Log intensity;
  \item Flag for each feature(0 for control or bad spot, = 1 for valid measurement);
\end{itemize}

The Sample ID used in the gene expression tables correspond to those used
in the clinical data table (archived in the \texttt{nejm\_table1.zip} compressed file).

The code used to assemble the \Rclass{ExpressionSet} instance for the van't Veer
cohort contained in the \Rpackage{seventyGenesData} {\R-\Bioc} package is shown below.
Since gene expression from the original van't Veer and Van de Vijver cohorts was
provided in the form of pre-processed, normalized, and summarized values,
pre-processing is not required.
...
<<assembelVanDeVijver, eval=TRUE, echo=TRUE, cache=FALSE>>=
##################################################
###Load the library
require(Biobase)
require(readxl)
...
<<assembelVanDeVijve1, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check presence of dowloaded file
...
<<assembelVanDeVijver2, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Read phenotypic information
...
<<extractColumns2, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Define the function
...
<<assembelVanDeVijver3, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembelVanDeVijver4, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
...
<<assembelVanDeVijver5, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<assembelVanDeVijver6, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
...
<<assembelVanDeVijver7, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Extract P-value
...
<<assembelVanDeVijver8, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
...
<<assembelVanDeVijver9, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Extract Intensity
...
<<assembelVanDeVijver10, eval=TRUE, echo=TRUE, cache=FALSE>>=
###Check order
...
<<assembelVanDeVijver11, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Patients' phenotypes in the \Robject{vantVeer } \Rclass{ExpressionSet}}

The code below is to add data set information to the \Robject{vantVeer}
\Rclass{ExpressionSet}.

<<addSetInfo, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<addSetInfo1, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
as opposed to patients who remained disease free for at least five years  (``good'' prognosis).

<<ttmVentVeer, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Patients' phenotypes in the \Robject{vanDeVijver } \Rclass{ExpressionSet}}

The {\R} code below is to create prognostic groups using recurrence information defined
as the occurrence of a metastasis as first event within five years from surgery (``bad'' prognosis),
as opposed to patients who remained disease free for at least five years  (``good'' prognosis).x

<<ttmVanDeVijver, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
<<ttmVanDeVijver2, eval=TRUE, echo=TRUE, cache=FALSE>>=
...
\Rmethod{pData} component of the ExpressionSet.

<<ttmVanDeVijver3, eval=TRUE, echo=TRUE, cache=FALSE>>=
